...
import { Lock, Cpu, Sparkles, Terminal, AlertCircle, CheckCircle2, Loader2, ArrowRight, ShieldAlert, Check } from 'lucide-react';
...
interface ProofGeneratorProps {
  ...
  proverLog: string;
  onGenerateProof: (privateAmountUSD: number, forceInvalidSig?: boolean) => void;
...
export const ProofGenerator: React.FC<ProofGeneratorProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onGenerateProof(assetInput, false);
  };

  const handleTestInvalidSig = () => {
    if (!wallet.isConnected) {
      onConnectWallet();
      return;
    }
    onGenerateProof(assetInput, true);
  };
    ...
    <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-purple-500/20 shadow-2xl relative overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          {/* Presets & Evaluator Test Scenarios */}
          <div className="space-y-2 mt-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="px-2.5 py-1 text-xs font-mono rounded-lg bg-midnight-900 border border-slate-700 hover:border-purple-400 text-slate-300 hover:text-white transition-all flex items-center space-x-1 cursor-pointer"
              >
                <Check className="w-3 h-3 text-emerald-400" />
                <span>$150,000 (Valid Proof)</span>
              </button>
              <button
                type="button"
                disabled={isRunning}
                onClick={() => handlePreset(65000)}
                className="px-2.5 py-1 text-xs font-mono rounded-lg bg-midnight-900 border border-slate-700 hover:border-rose-400 text-slate-300 hover:text-rose-300 transition-all flex items-center space-x-1 cursor-pointer"
              >
                <AlertCircle className="w-3 h-3 text-rose-400" />
                <span>$65,000 (Tests Invariant Failure)</span>
              </button>
              <button
                type="button"
                disabled={isRunning}
                onClick={handleTestInvalidSig}
                className="px-2.5 py-1 text-xs font-mono rounded-lg bg-midnight-900 border border-amber-500/40 hover:border-amber-400 text-amber-300 hover:text-amber-200 transition-all flex items-center space-x-1 cursor-pointer"
              >
                <ShieldAlert className="w-3 h-3 text-amber-400" />
                <span>Test Forged Signature</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div>
          {!wallet.isConnected ? (
            ...
            <button
              type="submit"
              disabled={isRunning}
              className={`w-full py-4 rounded-2xl font-bold text-base transition-all shadow-xl flex items-center justify-center space-x-2 cursor-pointer ${
                isRunning
